...
- Claude Code writes a `worktree-state` record into the transcript when a session enters or leaves a worktree (`EnterWorktree` / `ExitWorktree`, `claude -w`); the latest one wins. Without one, the session's shell returns to its start directory after every command, so its tool calls decide: the last file it edited, `cd` or `git -C` directory, or recorded cwd change.
- Codex records a thread's cwd once and never moves it, so the rollout's tool calls are scanned for the last `workdir`, `cd` or `git -C` directory.
- Either way only directories inside a git repository count (a scratch or container path does not move a session), a directory inside a linked worktree names it, and one in a main checkout puts the session back there.
...
- A Codex tab started fresh (New Codex, or Codex's own +) keeps the address of a new thread after its thread exists, so a reload would bring it back blank. Which thread each such tab showed is remembered per window, and after a reload that tab is reopened on its thread in the same slot.
...
- Codex threads come from `state_*.sqlite` via `node:sqlite` (read-only), a live thread's turn status from its rollout (the app-server daemon records no turns in `thread_history_*.sqlite`), liveness from `thread-writer-locks/` (on Linux, only locks the kernel lists in `/proc/locks` as held by a Codex process count; leftover unlocked files are ignored). On a host without `node:sqlite` it falls back to scanning the rollout files.
...
| `agentSessions.keepSessionsRunning` | `false` | keep Claude Code and Codex panel sessions running when their window closes (below); on in the phone window |

## Keeping sessions running

A panel's process belongs to its window: closing the window, a phone's browser dropping the page, or a server restart
ends the turn mid-tool. With `agentSessions.keepSessionsRunning` on, the extension points each installed tool at a
launcher in `~/.agent-sessions/bin`, so its sessions run outside the window. A window opened before the setting took
effect keeps its old process until it reloads; in the phone view the setting lives in the browser, so the first page
load after installing writes it and the next one uses it.

**Claude Code** (`claudeCode.claudeProcessWrapper`) runs the panel's `claude` under a session host
(`~/.agent-sessions/host`, a systemd user unit where available).
...
sessions already running carry on to their end.
...
- Phone mode moves the extension's views into a secondary side bar container and maximizes it, which is the workbench's
  ...
  tabs open behind the list, since closing a panel ends that session's process unless `agentSessions.keepSessionsRunning`
  keeps it running. The server keeps a closed page's extension host for five minutes, not VS Code's three hours, so
